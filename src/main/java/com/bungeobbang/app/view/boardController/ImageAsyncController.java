...
public class ImageAsyncController {
    ...
    public @ResponseBody String addImage(ServletContext servletContext, @RequestBody ImageFileDTO imageFileDTO) {
        ...
        try {//프로그램 비정상 종료를 막을 try catch
            fileName = FileUtil.insertFile(servletContext, FOLDER_PATH+imageFileDTO.getBoardFolder(), file, FileUtil.createFileName());
        } catch (NullPointerException | IllegalArgumentException e) {
            log.error("log: addImage - error insertFile() NPE or Illeagl : "+e.getMessage());
            return null;
        }
        src = servletContext.getContextPath() + FOLDER_PATH + imageFileDTO.getBoardFolder() + "/" + fileName;
