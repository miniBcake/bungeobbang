...
public class StoreMenuDAO {
	...
	final String INSERT = "INSERT INTO BB_STORE_MENU (STORE_MENU_NUM, STORE_NUM, STORE_MENU_NORMAL, STORE_MENU_VEG, STORE_MENU_MINI, STORE_MENU_POTATO, STORE_MENU_ICE, STORE_MENU_CHEESE, STORE_MENU_PASTRY, STORE_MENU_OTHER) " +
			"VALUES ((SELECT COALESCE(MAX(STORE_MENU_NUM), 0) + 1 FROM BB_STORE_MENU), ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public boolean insert(StoreMenuDTO storeMenuDTO) { // 신규등록가게 메뉴추가
		...
		try { 
			...
			pstmt.setString(2, storeMenuDTO.getStoreMenuNormal());       // 팥/슈프림 붕어빵 판매 여부(Y/N) 입력
	...
	public boolean update(StoreMenuDTO storeMenuDTO) {
		...
		try {
			...
			//[4] 인자값으로 받은 데이터 쿼리문에 삽입
			pstmt.setString(1, storeMenuDTO.getStoreMenuNormal());      // 팥/슈프림 붕어빵 판매 여부(Y/N) 입력
