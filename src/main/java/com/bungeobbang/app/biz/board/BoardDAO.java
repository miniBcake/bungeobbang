...
public class BoardDAO {
	...
	// INSERT 쿼리들
	// 게시글 작성
	private final String INSERT = "INSERT INTO BB_BOARD(BOARD_TITLE, BOARD_CONTENT, BOARD_FOLDER, BOARD_OPEN, BOARD_CATEGORY_NUM, MEMBER_NUM) "
			+ "VALUES(?,?,?,?,?,?)";

	// 상품 게시글 작성
	private final String INSERT_PRODUCT = "INSERT INTO BB_BOARD(PRODUCT_NUM, BOARD_TITLE, BOARD_CONTENT, BOARD_FOLDER, BOARD_CATEGORY_NUM) "
			+ "VALUES(?,?,?,?,?)";

	// 상점 게시글 작성
	private final String INSERT_STORE = "INSERT INTO BB_BOARD(BOARD_TITLE, BOARD_CONTENT, BOARD_FOLDER, STORE_NUM) "
			+ "VALUES(?,?,?,?)";

	// UPDATE 쿼리들
	private final String UPDATE = "UPDATE BB_BOARD SET BOARD_TITLE = ?, BOARD_CONTENT = ?, BOARD_OPEN = ? "
			+ "WHERE BOARD_NUM = ?";
	...
	// SELECTALL 쿼리
	private final String SELECTALL = "SELECT @rownum := @rownum + 1 AS RN, BOARD_NUM, BOARD_TITLE, BOARD_CONTENT, "
			...
			+ "FROM BB_VIEW_BOARD_JOIN, (SELECT @rownum := 0) AS r " + "WHERE 1=1 ";

	private final String SELECTALL_ENDPART = "ORDER BY BOARD_NUM DESC " + "LIMIT ?, ?";

	// SELECTALL_HOT 쿼리
	private final String SELECTALL_HOT = "SELECT BOARD_NUM, BOARD_TITLE, BOARD_CONTENT, BOARD_OPEN, BOARD_DELETE, "
			+ "BOARD_CATEGORY_NUM, BOARD_CATEGORY_NAME, MEMBER_NUM, MEMBER_NICKNAME, MEMBER_PROFILE_WAY, "
			+ "BOARD_WRITE_DAY, LIKE_CNT " + "FROM BB_VIEW_BOARD_JOIN "
			+ "WHERE BOARD_CATEGORY_NUM = ? AND LIKE_CNT >= ? " + "ORDER BY LIKE_CNT DESC LIMIT ? ";

	// SELECTALL_MYPAGE 쿼리
	private final String SELECTALL_MYPAGE = "SELECT @rownum := @rownum + 1 AS RN, BOARD_NUM, BOARD_TITLE, BOARD_CONTENT, "
	...
	// 인풋값 : 회원고유번호
	// 아웃풋 : 카테고리 고유번호, 게시물 고유번호,제목,작성일자, 좋아요수, 댓글수
	private final String SELECTALL_MYBOARD = "SELECT \n" + "    B.BOARD_CATEGORY_NUM, \n" + "    B.BOARD_NUM, \n"
			+ "    B.BOARD_TITLE, \n" + "    B.BOARD_CONTENT, \n" + "    B.BOARD_WRITE_DAY,\n"
			+ "    COUNT(DISTINCT L.LIKE_NUM) AS LIKE_COUNT,   -- 좋아요 수	\n"
			+ "    COUNT(DISTINCT R.REPLY_NUM) AS REPLY_COUNT  -- 댓글 수		\n" + "FROM \n" + "    BB_BOARD B\n"
			+ "LEFT JOIN \n" + "    BB_LIKE L ON B.BOARD_NUM = L.BOARD_NUM\n" + "LEFT JOIN \n"
			+ "    BB_REPLY R ON B.BOARD_NUM = R.BOARD_NUM\n" + "WHERE \n" + "    B.MEMBER_NUM = ?\n" + "GROUP BY \n"
			+ "    B.BOARD_CATEGORY_NUM, \n" + "    B.BOARD_NUM, \n" + "    B.BOARD_TITLE, \n"
			+ "    B.BOARD_CONTENT, \n" + "    B.BOARD_WRITE_DAY\n" + "ORDER BY \n" + "    B.BOARD_WRITE_DAY DESC;\n";

	// SELECTONE 쿼리
	private final String SELECTONE = "SELECT BOARD_NUM, BOARD_TITLE, BOARD_CONTENT, BOARD_OPEN, BOARD_DELETE, "
			+ "BOARD_CATEGORY_NUM, BOARD_CATEGORY_NAME, MEMBER_NUM, MEMBER_NICKNAME, MEMBER_PROFILE_WAY, "
			+ "BOARD_WRITE_DAY, BOARD_FOLDER " + "FROM BB_VIEW_BOARD_JOIN WHERE BOARD_NUM = ?";

	// SELECTONE_SEARCH 쿼리
	private final String SELECTONE_SEARCH = "SELECT COUNT(*) AS CNT FROM BB_BOARD bb "
	...
	private final String SELECTONE_MY = "SELECT COUNT(*) AS CNT FROM BB_BOARD bb "
			+ "JOIN BB_MEMBER bm ON bb.MEMBER_NUM = bm.MEMBER_NUM " + "WHERE MEMBER_NUM = ?";
	...
	public boolean insert(BoardDTO boardDTO) {
		...
		try {
			if (boardDTO.getCondition().equals("BOARD_INSERT")) {
				...
				query = INSERT;
				rs = jdbcTemplate.update(query, boardDTO.getBoardTitle(), boardDTO.getBoardContent(),
						boardDTO.getBoardFolder(), boardDTO.getBoardOpen(), boardDTO.getBoardCategoryNum(),
				...
				rs = jdbcTemplate.update(query, boardDTO.getProductNum(), boardDTO.getBoardTitle(),
						boardDTO.getBoardContent(), boardDTO.getBoardFolder(), PRODUCT // 상품 카테고리 번호
				);
				...
				query = INSERT_STORE;
				rs = jdbcTemplate.update(query, boardDTO.getBoardTitle(), boardDTO.getBoardContent(),
						boardDTO.getBoardFolder(), boardDTO.getStoreNum());
			} else {
				// 컨디션값 오류
	...
	public boolean update(BoardDTO boardDTO) {
		...
		if (boardDTO.getCondition().equals("BOARD_UPDATE")) {
			...
			query = UPDATE;
			args = new Object[] { boardDTO.getBoardTitle(), boardDTO.getBoardContent(), boardDTO.getBoardOpen(),
					boardDTO.getBoardNum() };
		} else if (boardDTO.getCondition().equals("ADMIN_DELETE")) {
			// 관리자 삭제
			System.out.println("log: Board update : ADMIN_DELETE");
			query = UPDATE_DEL;
			args = new Object[] { boardDTO.getBoardNum() };
		} else {
	...
	public List<BoardDTO> selectAll(BoardDTO boardDTO) {
		...
		if (boardDTO.getCondition().equals("FILTER_BOARD")) {
			...
			args = new Object[] { boardDTO.getBoardCategoryNum(), MINLIKE, SHOWHOTBOARD };
			query = SELECTALL_HOT;
		} else if (boardDTO.getCondition().equals("MY_BOARD")) {
			// 내 게시글 모아보기
			System.out.println("log: Board selectAll : MY_BOARD");
			args = new Object[] { boardDTO.getMemberNum(), boardDTO.getStartNum(), boardDTO.getEndNum() };
			query = SELECTALL_MYPAGE;
		} else if (boardDTO.getCondition().equals("SELECTALL_MYBOARD")) {
			System.out.println("log: Board SELECTALL_MYBOARD : " + boardDTO.toString());
			// 카테고리 고유번호, 게시물 고유번호, 제목, 내용, 작성자, 작성일자
			args = new Object[] { boardDTO.getMemberNum() };
			return jdbcTemplate.query(SELECTALL, args, new MyBoardRowMapper());
		}
	...
	// 내 게시물 모두 조회 mapper 메서드
	class MyBoardRowMapper implements RowMapper<BoardDTO> {
		@Override
		public BoardDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
			System.out.println(" BoardDTO selectAll mapRow() MyBoardRowMapper start");
			BoardDTO data = new BoardDTO();
			data.setBoardCategoryNum(rs.getInt("BOARD_CATEGORY_NUM")); // 카테고리 고유번호
			data.setBoardNum(rs.getInt("BOARD_NUM")); // 게시물 고유번호
			data.setBoardTitle(rs.getString("BOARD_TITLE")); // 게시물 제목
			data.setBoardContent(rs.getString("BOARD_CONTENT")); // 게시물 내용
			data.setBoardWriteDay(rs.getString("BOARD_WRITE_DAY")); // 게시물 작성일자
			data.setLikeCnt(rs.getInt("LIKE_COUNT")); // 게시물 좋아요 수
			data.setCnt(rs.getInt("REPLY_COUNT")); // 게시물 댓글 수
			System.out.println(" BoardDTO selectAll mapRow() MyBoardRowMapper end");
			return data;
		}
	}
}