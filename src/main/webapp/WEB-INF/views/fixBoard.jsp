			...
			<div class="row mt-5">
				...
				<div class="col-md-9">
					...
					<section class="content-header">
						<h2 class="content-title">게시물 수정</h2>
						<p class="content-description">게시글을 수정해주세요!</p>
					</section>

					<section>
						<!-- 게시글 수정 폼 -->
						<form id="postForm" action="updateBoard.do" method="post"
							...
							<input type="hidden" id="boardNum" name="boardNum"
								...
								name="boardFolder" value="${board.boardFolder}">

							<div class="form-group">
								<label for="boardCateName">일반/문의</label>
								<!-- 게시판 카테고리 선택 옵션 -->
								<select id="boardCateName" name="boardCategoryName"
									class="form-control">
									<option value="boardList"
										<c:if test="${board.boardCategoryName == 'boardList'}">selected</c:if>>일반</option>
									<option value="noticeBoard"
										<c:if test="${board.boardCategoryName == 'noticeBoard'}">selected</c:if>>문의</option>
								</select>
							</div>
							...
							<div id="postSecretGroup"
								...
								<label class="custom-checkbox"> <input type="checkbox"
									...
									class="checkmark"></span> <span class="checkbox-label">비공개
										게시물</span>
								</label> <input type="hidden" id="hiddenBoardOpen" name="boardOpen"
									value="Y">
								<!-- 비공개 여부를 위한 hidden 필드 -->
							</div>
							...
							<div class="form-group mt-3">
								...
								<textarea id="boardContent" name="boardContent"
									style="display: none;"><c:out
										value="${board.boardContent}" /></textarea>
							</div>
							<!-- 폼 제출 버튼 -->
